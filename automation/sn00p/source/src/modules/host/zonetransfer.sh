...
#                                                                              #
# FILE                                                                         #
# src/modules/host/zonetransfer.sh                                             #
#                                                                              #
# DATE                                                                         #
# 09/02/2012                                                                   #
#                                                                              #
...
# we need in most of our routines, so we make global
domain="../default/domain.log"
nameserver="../default/nameserver.log"


# dns zone transfer via host
# TOOLS: host
do_zonetransfer_host()
{
    for domain in `cat ${domain}`
    do
        for nameserver in `cat ${nameserver}`
        do
            host -l "${domain}" "${nameserver}" 2>&1
        done
    done
...
do_zonetransfer_dig()
{
    ...
    do
        for nameserver in `cat ${nameserver}`
        do
            dig axfr "@${nameserver}" "${domain}" 2>&1
        done
    done
...
do_dnswalk()
{
    ...
    for domain in `cat ${domain}`
    do
        dnswalk ${opts} "${domain}." 2>&1
    done

    return ${SUCCESS}
}

# dns zone transfer via dnsenum
# TOOLS: dnsenum
do_dnsenum()
{
    dnsenum ${DHOST} 2>&1
