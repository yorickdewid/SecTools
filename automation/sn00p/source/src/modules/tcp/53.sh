...
# try to get version of bind via host
# TOOLS: host
do_version_bind_host()
{
    opts="-c chaos -t txt version.bind"
...
# try to get version of bind via dig
# HOST: dig
do_version_bind_dig()
{
    opts="version.bind txt chaos"
...
do_snoop_cache()
{
    ...
    # make first request
    dig @${DHOST} ${site} ${opts} 2>&1 | grep -i "answer"

    # just newline
    echo

    # make second request (ANSWER flag)
    dig @${DHOST} ${site} ${opts} 2>&1 | grep -i "answer"
