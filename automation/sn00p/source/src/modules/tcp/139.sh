...
# scan for netbios infos via nbtscan
# TOOLS: nbtscan
do_nbtscan()
{
    opts="-v -r"

    nbtscan ${opts} ${DHOST} 2>&1

    return ${SUCCESS}
}


# scan for netbios infos via nmbscan
# TOOLS: nmbscan
do_nmbscan()
{
    opts="-h"

    nmbscan ${opts} ${DHOST} 2>&1

    return ${SUCCESS}
}


# try to get infos over msrpc endpoint mapper via rpcdump.py
# TOOLS: rpcdump.py
do_rpcdump()
{
    rpcdump.py ${DHOST} "139/SMB" 2>&1

    return ${SUCCESS}
}


# try to get as much as possible via enum4linux
# TOOLS: enum4linux
do_enum4linux()
{
    opts="-a"

    enum4linux ${opts} ${DHOST} 2>&1
