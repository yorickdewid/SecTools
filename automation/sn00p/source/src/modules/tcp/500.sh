...
# connect and try to get infos about logged in users
# TOOLS: ike-scan
do_ikescan()
{
    opts=""
...
# probe ike requests and enumerate ike related stuff via IKEProber.pl
# TOOLS: ikeprober
do_ikeprober()
{
    opts=""

    perl ikeprober -d ${DHOST} 2>&1

    return ${SUCCESS}
}


# capture authentication messages (aggressive mode IKE) via ikecrack-snarf
# TOOLS: ikecrack-snarf-1.00.pl
do_ikecrack_snarf()
{
    opts=""

    ikecrack-snarf-1.00.pl "${DHOST}.${DPORT}" 2>&1
