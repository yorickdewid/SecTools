...
# fingerprint drupal modules / versions
# TOOLS: DrupalScan
do_drupalscan()
{
    opts=""

    echo "DrupalScan against ${DHOST}" 2>&1
    DrupalScan ${opts} -u ${DHOST} 2>&1

    echo "DrupalScan against ${URL}" 2>&1
    DrupalScan ${opts} -u ${URL} 2>&1

    return ${SUCCESS}
}


# drupal modules enumerator
# TOOLS: dpscan
do_dpscan()
{
    opts=""
