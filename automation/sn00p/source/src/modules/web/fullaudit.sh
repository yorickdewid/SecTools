...
# crawl and scan webpage with wapiti
# TOOLS: wapiti
do_wapiti()
{
    opts="--module '-all,xss,crlf,sql,file,htaccess,blindsql,permanentxss'
    -t 3 -f txt -o /dev/stdout"

    wapiti.sh ${URL} ${opts} 2>&1

    return ${SUCCESS}
}


# crawl and scan webpage with uniscan
# TOOLS: uniscan
do_uniscan()
{
    opts="-q -w -e -d -s"

    # no need to log uniscan output (uniscan report is enough)
    uniscan ${opts} -u ${URL} 2>&1

    return ${SUCCESS}
}


# crawl and scan webpage with skipfish
# TOOLS: skipfish
do_skipfish()
{
    # note: default crawl depth level: 3
    opts="-u -i 5 -t 5 -d 3 -o ${URL}_skipfish"

    # no need to log skipfish output (skipfish report is enough)
    skipfish ${opts} ${URL} > ${VERBOSE} 2>&1
    echo "skipfish has created own report, see ${URL}_skipfish/"

    return ${SUCCESS}
}


# crawl and scan webpage with arachni
# TOOLS: arachni
do_arachni()
{
    # note: default crawl depth level: 3
    opts="--user-agent=yomama --depth=3 --report=stdout"

    arachni ${opts} ${URL} 2>&1
