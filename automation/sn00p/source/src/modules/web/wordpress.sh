...
do_plecost()
{
    opts="-i wp_plugin_list.txt -s 5 -M 10 -t 8 -o /dev/stdout"

    plecost ${opts} ${DHOST} 2>&1

    return ${SUCCESS}
}


# scan wordpress (enumerate vuln plugins and themes, etc.)
# TOOLS: wpscan
do_wpscan()
{
    opts="-f -t 8 --wp-content-dir wp-content"

    # echo, because wpscan is asking for plugin dir interactively,
    # if not found (yes, 'yes' is available, fuck that...)
    echo -e "y\r\n" | wpscan.sh ${opts} -u ${DHOST} 2>&1
