...
# map whole webapplication and do quick security scans
# TOOLS: golismero
do_golismero()
{
    opts="-e dns,dns_malware,fingerprint_web,punkspider,robots,spider"
    opts="${opts} --full --no-color -q -o -"

    golismero ${opts} scan ${URL} 2>&1

    return ${SUCCESS}
}


# just crawl the webpage and save website data for later usage (uwss modules)
# TOOLS: uwss.sh
do_uwss()
{
    opts="-q -c"

    # we do not need any log results. we are only interested in website data
    # results will be saved under /tmp/uwss.dat
    uwss.sh ${opts} -u ${URL} > /dev/null 2>&1

    echo "crawled data with uwss" 2>&1
