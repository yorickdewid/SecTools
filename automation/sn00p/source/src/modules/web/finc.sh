...
# crawl and scan website for file inclusion bugs with fimap
# TOOLS: fimap
do_fimap()
{
    opts="-H -4 -b -v 1 -d 3"

    fimap ${opts} -u ${URL} 2>&1

    return ${SUCCESS}
}

# test for file inclusion bugs with uwss
# TOOLS: uwss
do_uwss_finc()
{
    opts1="-q -m lfi"
    ...
    uwss.sh ${opts1} -u ${URL} 2>&1

    # remote file inclusion
    uwss.sh ${opts2} -u ${URL} 2>&1
