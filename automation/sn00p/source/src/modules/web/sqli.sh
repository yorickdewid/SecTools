...
# test sql injection for single url with sqlmap (GET only!)
# TOOLS: sqlmap
do_sqlmap()
{
    opts="--user-agent=yomama --timeout=5 --retries=2 --level=2 --risk=3
    --time-sec=3 -f --current-user --current-db --hostname --threads=4"

    sqlmap ${opts} -u ${URL} 2>&1

    return ${SUCCESS}
}


# scan website for sql injection bugs with uwss
# TOOLS: uwss.sh
do_uwss_sqli()
{
    opts="-m sql -q"

    uwss.sh ${opts} -u ${URL} 2>&1
